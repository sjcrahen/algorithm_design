...
public class DLX {
    ...
    private void search(int k) {
        if (head.R == head) { // base case
            //printSolution();
            solutions++;
        } else {
            ColumnNode colHead = selectColumn();
            colHead.cover();
            for (DataNode colNode = colHead.D; colNode != colHead; colNode = colNode.D) {
                solution.add(colNode);
                for (DataNode rowNode = colNode.R; rowNode != colNode; rowNode = rowNode.R)
                    rowNode.C.cover();
                // recursive step
                search(k+1);
                
            // --------- backtrack ------------ //
                solution.remove(colNode);
                colHead = colNode.C;
                for (DataNode rowNode = colNode.L; rowNode != colNode; rowNode = rowNode.L)
                    rowNode.C.uncover();
            }
            colHead.uncover();
            // ------ backtrack complete ------ //
